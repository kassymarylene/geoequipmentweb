    ...
    <>
      ...
      <div className="bg-slate-950 text-white">
        <section className="relative overflow-hidden px-6 py-16">
          <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/30 via-transparent to-slate-950/80" />
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4">
                {testimonialMetrics.map((metric) => (
                  <div key={metric.label} className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm">
                    <p className="text-2xl font-semibold text-white">{metric.label}</p>
                    <p className="text-xs uppercase tracking-[0.4em] text-white/70">{metric.detail}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative h-64 w-full overflow-hidden rounded-3xl border border-white/30 bg-gradient-to-br from-white/20 to-transparent shadow-xl shadow-cyan-700/40 lg:h-72">
              <Image
        ...
        <section className="px-6 py-12">
          <div className="mx-auto max-w-6xl space-y-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.6em] text-cyan-200">Property Types</p>
            <h2 className="text-2xl font-semibold text-white">Tailored experiences by asset class</h2>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {propertyHighlights.map((item) => (
              <article
                key={item}
                className="rounded-3xl border border-white/10 bg-white/10 p-6 text-sm text-white shadow-lg shadow-black/30 backdrop-blur"
              >
                {item}
              </article>
            ))}
        ...
        <section className="px-6 pb-16">
          <div className="mx-auto max-w-6xl space-y-6">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <h3 className="text-2xl font-semibold text-white">Featured Deliverables</h3>
              <p className="text-sm text-white/70">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {productTiles.map((product) => (
                ...
                >
                  <div className="relative h-32 rounded-2xl border border-white/20 bg-white/10">
                    <Image
